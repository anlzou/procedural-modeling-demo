...
- **☀️ 时段切换** — 午夜 → 晨曦 → 正午 → 黄昏 → 午夜，颜色平滑插值；实时同步系统时间或手动调节，太阳/月亮位置驱动光照与相机重置朝向
- **🌧️ 天气系统** — 晴朗 / 下雪 / 下雨 / 风暴 四档切换；雨滴落水产生扩散波纹、随机风向使强风摆动、风暴模式附带闪电（随机折线闪电 + 天空/海面双闪泛光）；进入风暴自动拉满海况并 2.0x 加速，平滑过渡、退出恢复
- **☄️ 流星系统** — 晴朗夜晚自动（间隔 5~17s）或按 **M** 键连发触发；**红 / 白双色**（对应 effect_01.wgsl 的红白火焰，每颗随机）；**两种轨迹模式**（控制面板 ☄️ 流星 实时配置）：**随机角**（左右两侧互相飞：左→右 / 右→左随机交替横穿屏幕，距离镜头很远 1500~3200，出画后淡出）与 **辐射点**（每批 1~6 颗从屏幕上方同一辐射点附近出现，扇形发散射向远方，可调发散范围）；**火焰动画开关**（默认关闭 = 直接跳过干涉纹理亮格、退化为平滑彗尾+头部，彻底消除高速飞行时的闪烁方块；开启后恢复随时间流动的干涉火焰）；飞行速度较慢、拖尾更长，着色器内叠加**平滑彗尾基座**保证连贯拖尾；所有流星始终飞行在**海平面之上**，不会落入海面（避免落海闪烁）；流星池（最多 6 颗并发），每颗流星为朝向相机的公告牌四边形 + TSL 节点着色器程序化渲染拖尾火焰（干涉纹理累积 + 噪声扰动 + tanh 色调映射，采用 effect_01.wgsl 思路），拖尾对齐飞行方向并随距离远去缩小
...
- **控制面板** — 左上角 InfoPanel 集成海况/时段/天气/画质/鱼缸/鱼群控制 + 右下角性能监控与面板透明度；海况栏内置自动旋转 ON/OFF、时段栏内置实时同步 ON/OFF
