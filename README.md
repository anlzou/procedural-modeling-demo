...
- **☄️ 流星系统** — 晴朗夜晚自动（间隔 5~17s）或按 **M** 键连发触发；**红 / 白双色**（对应 effect_01.wgsl 的红白火焰，每颗随机）；**两种轨迹模式**（控制面板 ☄️ 流星 实时配置）：**随机角**（左右两侧互相飞：左→右 / 右→左随机交替横穿屏幕，距离镜头很远 1500~3200，出画后淡出）与 **辐射点**（每批 1~6 颗从屏幕上方同一辐射点附近出现，扇形发散射向远方，可调发散范围）；飞行速度较慢、拖尾更长，着色器内叠加**平滑彗尾基座**保证连贯拖尾；所有流星始终飞行在**海平面之上**，不会落入海面（避免落海闪烁）；流星池（最多 6 颗并发），每颗流星为朝向相机的公告牌四边形 + TSL 节点着色器程序化渲染拖尾火焰（干涉纹理累积 + 噪声扰动 + tanh 色调映射，采用 effect_01.wgsl 思路），拖尾对齐飞行方向并随距离远去缩小
