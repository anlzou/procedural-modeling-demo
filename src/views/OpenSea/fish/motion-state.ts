...
export function updateFishMotionState(
  ...
  if (!fish || !nextVelocity || dt <= 0) return
  ...
  if (previousDirection.lengthSq() <= 0.000001 || nextDirection.lengthSq() <= 0.000001) {
  ...
  fish.bank = dampAngle(fish.bank ?? 0, targetBank, fishConfig.bankResponse, dt)
}

function updateSwimPhase(fish: FishState, currentSpeed: number, nextSpeed: number, dt: number): void {
  const accelerationDrive = updateAccelerationSwimDrive(fish, currentSpeed, nextSpeed, dt)
...
function readSwimFrequency(swimDrive: number): number {
  const frequency = THREE.MathUtils.lerp(fishConfig.swimFrequencyMin, fishConfig.swimFrequencyMax, swimDrive)
  const minInterval = fishConfig.swimTailBeatMinIntervalSeconds ?? 0
  if (minInterval <= 0) return frequency
  return Math.min(frequency, 1 / minInterval)
}

function updateAccelerationSwimDrive(
  fish: FishState, currentSpeed: number, nextSpeed: number, dt: number,
): number {
  const acceleration = (nextSpeed - currentSpeed) / Math.max(0.0001, dt)
  const triggerDrive = THREE.MathUtils.smoothstep(
    acceleration, fishConfig.swimAccelerationThreshold, fishConfig.swimAccelerationFull,
  )
  const duration = Math.max(0.0001, fishConfig.swimAccelerationPulseSeconds)
  fish.swimAccelerationTime = Math.max(0, (fish.swimAccelerationTime ?? 0) - dt)
  if (acceleration <= 0) {
...
function updateCurveBend(
  fish: FishState, previousDirection: THREE.Vector3, nextDirection: THREE.Vector3,
  speed: number, dt: number, scratch: FishMotionScratch,
): void {
...
function dampCurveBend(
  fish: FishState, targetCurve: THREE.Vector3 | null, dt: number, scratch: FishMotionScratch,
): void {
  if (!fish.curveBendWorld) fish.curveBendWorld = new THREE.Vector3()
...
function readTurnSwimDrive(curveBendWorld: THREE.Vector3 | undefined): number {
  if (!curveBendWorld || curveBendWorld.lengthSq() <= 0.000001) return 0
  const normalizedCurve = THREE.MathUtils.clamp(
    curveBendWorld.length() / Math.max(0.0001, fishConfig.curveDeformationMax), 0, 1,
  )
