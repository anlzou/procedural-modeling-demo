...
export function readFishDirection(
  ...
  if (velocity && velocity.lengthSq() > 0.000001) return target.copy(velocity).normalize()
  const introDropDirection = fish?.introDropDirection
  if (introDropDirection && introDropDirection.lengthSq() > 0.000001) return target.copy(introDropDirection).normalize()
...
export function writeFishOrientationQuaternion(
  ...
  tmpDorsal.normalize()
  const bank = THREE.MathUtils.clamp(fish.bank ?? 0, -fishConfig.maxBankAngle, fishConfig.maxBankAngle)
